...
export function SongCard({ item, onPlay, className }: SongCardProps) {
  const imageUrl = item.image.find(q => q.quality === '150x150')?.link || item.image[0].link;
  const title = item.name || item.title;
  const description = item.type === 'song' ? item.primaryArtists : (item.subtitle || item.description || '');

  return (
    <div 
        className={cn('group cursor-pointer transform transition-transform duration-300 hover:scale-105', className)}
        onClick={() => onPlay(item)}
    >
